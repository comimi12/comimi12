...
| `secure-doc-intake` | 보안문서(FASSO 등) 정식 반출분을 00-inbox/secure-export/에서 무결성 확인 후 변환·정리. **암호화/DRM 해제는 하지 않음** — 이미 평문화된 파일만 대상 | "보안문서 반입", "FASSO 파일 정리" |
...
- `secure-doc-intake` — Python 표준 라이브러리만 사용, 추가 패키지 설치 불필요. 무결성 게이트 통과 후 `excel-to-csv`/`pdf-to-md`/`hwpxskill`에 위임하므로 해당 스킬들의 의존성은 그대로 적용됨.
