...
| 탭 | 내용 |
|----|------|
| 📅 일별 리서치 | **날짜별 리뷰 유입 확인**(전일 체크용). 작성일/수집일 기준 전환, 해당일·7일·30일 범위, 브랜드·감성·검색 필터, 리뷰 원문 카드, CSV 다운로드. 아래 **일별 리서치** 섹션 참조 |
...
월별 집계와 달리 **"언제 올라온 리뷰인가"** 를 본다. `build.py` 가 `data.js` 의 `reviews.daily` 를 만든다.
