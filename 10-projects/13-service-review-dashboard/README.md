...
| 탭 | 내용 |
|----|------|
| 📅 일일리뷰유입요약 | **기간을 정해 리뷰 원문 전문(全文) 확인**. 작성일/수집일 기준 전환, 시작일~종료일 기간 선택, 브랜드·감성·검색 필터, CSV 다운로드. 아래 **일일리뷰유입요약** 섹션 참조 |
...
- **네이버 보충(merged)**: `merged_reviews_*.csv`(collect.py 대량수집 양식)의 **네이버 행도 합산**한다. 일일 자동수집(`naver_collected_*.csv`)이 실패·부분수집된 달을 이걸로 메운다. 같은 리뷰는 (매장·방문일·내용)으로 중복 제거되고, 사전분류 감성이 있는 `naver_collected` 쪽이 우선 채택된다.
  - ⚠️ merged 양식에는 감성 라벨이 없어 **키워드 분류**를 쓴다. 라벨 8,966건으로 검증 시 **정밀도 69.8% · 재현율 44.4%** → 그 달 불만은 실제보다 적게 잡히고 잡힌 것의 30%는 오탐. **프로그램 원본 파일이 들어오면 그 달은 자동으로 프로그램 100%로 대체**되므로 원본을 받는 즉시 넣을 것.
  - 📌 2026-08-04 이전 빌드는 merged 의 네이버를 통째로 버려서 **7월이 794건으로 과소집계**됐다(실제 4,331건). 같은 유형의 누락을 의심하면 `merged_reviews_*.csv` 의 `source=naver` 행수와 `data.js` 월 합계를 대조할 것.
- **부분 월 자동 제외**: 소량 월(2026년 이후 월 건수 중앙값의 25% 미만)은 현재월/집계에서 제외. 수집 중인 당월이 25%를 넘기면 자동으로 다시 포함된다.
- **신규 오픈 매장**: 그 달 리뷰가 1건이라도 들어오면 자동 포함(매장명 코드 C/H/S/I/J로 분류). 별도 등록 불필요.

---

## 🔄 전체 자동 갱신 (평일 09:00, 팝업 확인형) — update_all.py

Windows 작업 **`ServiceReview_Daily_Update`** 가 **평일(월~금) 09:00** `prompt-update.ps1` 실행 → **팝업 "예" 클릭 시** `update_all.py` + `export_monthly.py`로 ①②③④⑤⑥ 전부 갱신 + 당월 분석 엑셀 생성 + 공유사이트 재배포.

- **주말 제외**: 토·일은 스케줄에 없음(노트북 OFF로 밀린 실행이 안 쌓임).
- **놓친 실행 보충**: 09:00에 노트북이 꺼져 있었으면 로그인하는 순간 팝업이 뜸(StartWhenAvailable).
- **팝업 스크립트**: `prompt-update.ps1` (UTF-8 BOM 저장 — PS 5.1 한글 깨짐 방지). "아니오" 누르면 그날 건너뜀.
- ⑤⑥(ECK)도 `update_all.py`에 포함되어 같은 9시 팝업에서 함께 갱신됨(별도 08:30 작업은 통합·제거됨).
...
- 지금 테스트(팝업 띄우기): `schtasks /Run /TN "ServiceReview_Daily_Update"`
- 작업 확인: `schtasks /Query /TN "ServiceReview_Daily_Update"`
- 작업 삭제: `schtasks /Delete /TN "ServiceReview_Daily_Update" /F`
- 설정: *놓친 실행 보충(StartWhenAvailable) + 실패 시 재시도(10분 간격 3회) + 절전 중 중단 안 함 + 배터리에서도 실행*. 팝업 표시를 위해 *Interactive(로그인 세션)* 로 동작.

> `run-daily.cmd`/`run-eck.cmd`는 무팝업 직접 실행이 필요할 때를 위한 보조 스크립트로 남겨둠(스케줄은 `prompt-update.ps1` 사용).
...
추이·집계 없이 **원문만** 본다. `build.py` 가 `data.js` 의 `reviews.daily` 를 만든다.

| 기준 | 의미 |
|------|------|
| **작성일** (기본) | 고객이 리뷰를 올린 날 (없으면 방문일) |
| **수집일** | 우리 자동수집이 그 리뷰를 **처음 가져온 날**(first_seen). `naver_collected_YYYYMMDD` 파일명 = 수집일 |

- **기간 선택**: 시작일 ~ 종료일 드롭다운으로 원하는 구간을 고른다(기본=최신 하루). 프리셋(7일/30일) 없음.
- **원문 전문**: 텍스트를 자르지 않고(옛 400자 컷 제거) 카드 높이 제한 없이 전부 표시.
- **보관 기간 `DAILY_WINDOW = 120`일**: 이 안이면 어느 날짜든 전문 조회 가능. `build.py` 상단 상수로 조절.
  (넓힐수록 data.js 커짐 — 120일 = 원문 약 1.6만 건, data.js 3.9MB)
- 성능상 기본 300건까지 렌더하고 "전체 N건 모두 보기" 버튼으로 전부 펼친다.
- 매장명은 인덱스로 접어 저장(`daily.stores` + `records[i][2]`).

> ⚠️ 최신성은 `collect_v2.py` 자동수집에 달려 있다. 아래 **collect_v2.py** 섹션 참조.

------|------|--------|
| **작성일** (기본) | 고객이 리뷰를 올린 날 (없으면 방문일) | 최근 180일 일별 집계 `by_day` |
| **수집일** | 우리 자동수집이 그 리뷰를 **처음 가져온 날**(first_seen) | `naver_collected_YYYYMMDD_*.csv` 파일명 = 수집일 |

- 원문(리뷰 텍스트)은 용량 때문에 **최근 62일만 보관**(`REC_WINDOW`). 그 이전은 집계·추이로만 조회된다.
  집계 범위는 180일(`AGG_WINDOW`). 둘 다 `build.py` 상단 상수로 조절.
- 매장명은 인덱스로 접어 저장(`daily.stores` + `records[i][2]`) — data.js 용량 관리용.
- 차트 막대를 클릭하면 그 날짜로 이동. 리뷰 0건인 날도 선택 가능.
- **전일 새로 들어온 리뷰**를 보려면 `수집일 기준` → 최신 날짜를 고르면 된다.

> ⚠️ 이 탭의 최신성은 `collect_v2.py` 자동수집에 달려 있다. 네이버 차단(405/429)으로 수집이
> 타임아웃되면 앞쪽 몇 개 매장만 수집되고 나머지는 비어 보인다 — `_logs/update_*.log` 의
> "리뷰 수집 타임아웃" 여부를 먼저 확인할 것.
...
- **DRM 내성**: 과거월 원본(`cs_*.xlsx`/`qscs_*.xlsx`)은 시간이 지나면 사내 DRM(DRMONE)으로 디스크에서 재암호화돼 다시 못 읽는다. `eck_build.py`는 PK(zip) 매직으로 진짜 xlsx만 파싱하고, 읽기 실패한 월은 **직전 `eck_data.js`의 정상값을 유지**(crash 안 함). 당월은 매일 새로 받으므로(평문) 항상 최신 반영. → `eck_data.js`를 함부로 지우지 말 것(과거월 캐시 소실).
