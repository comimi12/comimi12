...
"""
import sys, io, os, json, glob, datetime, calendar, re, subprocess, tempfile, hashlib
...
# ── Fasoo DRM 복호화 폴백 ──────────────────────────────────────────────
# 이 PC의 ECK Excel 내보내기는 사내 DRM으로 재암호화되어 openpyxl/pandas 가
# 직접 못 읽는다. Fasoo 에이전트가 살아있는 대화형 세션에서는 Excel COM 으로
# 열면 복호화되므로, PowerShell 로 첫 시트 값을 평문 CSV 로 덤프해 파싱한다.
# (비대화형 작업 스케줄러 등 Excel COM 이 안 뜨는 환경에선 실패 → 상위에서
#  이전 정상값으로 폴백하므로 회귀 없음.)
_PS_DUMP = r'''
...
def _decrypt_via_excel(path):
    """DRM 잠긴 xlsx → Excel COM 으로 첫 시트를 평문 CSV 로 덤프, 그 경로 반환.
    소스보다 새 캐시가 있으면 재사용(스케줄러 반복 실행 시 Excel 반복 기동 회피)."""
    key = hashlib.md5(os.path.abspath(path).encode("utf-8")).hexdigest()[:12]
    out = os.path.join(tempfile.gettempdir(), f"eck_drm_{key}.csv")
    if os.path.exists(out) and os.path.getmtime(out) >= os.path.getmtime(path):
        return out
    ps = _PS_DUMP.replace("__PATH__", path).replace("__OUT__", out)
    subprocess.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", ps],
                   check=True, capture_output=True, timeout=180)
    if not (os.path.exists(out) and os.path.getsize(out) > 0):
        raise RuntimeError("Excel COM 덤프 결과 없음")
    return out


def load_df(path):
    """정상 xlsx(PK) → pandas 직독. DRM 잠김 → Excel COM 복호화 CSV 파싱.
    둘 다 header=None DataFrame(문자열) 반환."""
    if is_xlsx(path):
        return pd.ExcelFile(path, engine="openpyxl").parse(0, header=None)
    csvp = _decrypt_via_excel(path)  # DRM → CSV
    return pd.read_csv(csvp, header=None, dtype=str, keep_default_na=False,
                       encoding="utf-8-sig")
...
def main():
    ...
    for mo in months:
        ...
        if os.path.exists(cs_path):
            # 정상 xlsx → 직독 / DRM 잠김 → Excel COM 복호화(load_df). 실패 시 이전값 유지.
            try:
